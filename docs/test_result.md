...
## 18. 单一 Dockerfile 合并后的本地 Compose 验证（2026-07-18）

将原 `Dockerfile.server` 的构建缓存与 `GOPROXY` 兜底并入根目录唯一 `Dockerfile` 后，在 Windows + Docker Desktop 上做完整栈回归。首次失败原因为既有 MySQL 数据卷密码与当前 `.env` 不一致；`docker compose down -v` 后重建通过。

| 检查项 | 结果 | 备注 |
|---|---|---|
| 镜像构建 | 通过 | `docker compose --env-file .env up -d --build --wait`，多阶段构建含 app 与 goose |
| Goose 迁移 | 通过 | migrate 退出码 0，成功迁移到 version 14 |
| 依赖健康 | 通过 | mysql / redis / rabbitmq 均为 healthy |
| 应用健康 | 通过 | app healthy；容器内 HEALTHCHECK 与 Compose `readyz` 探针均通过 |
| `/ping` | 通过 | HTTP 200 |
| `/live` | 通过 | HTTP 200 |
| `/readyz` | 通过 | HTTP 200 |
| `/metrics` | 通过 | HTTP 200 |
| 文档同步 | 通过 | `.env.example`、`docs/deploy.md`、`README.md` 与单一 Dockerfile 路径对齐 |

边界：本次只验证合并后的本地 Compose 路径，不替代云主机一键部署与长期运行证据。
