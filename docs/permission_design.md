...
- 管理员通过鉴权后访问商品写操作、库存写操作、库存流水和操作日志接口时，会写入 `operation_logs`。
- 操作日志记录 `user_id`、`username`、`action`、`method`、`path`、`route`、`http_status`、`request_id`、`client_ip`、`user_agent` 和 `created_at`。
- 操作日志不记录请求体，避免把密码、Token 或业务敏感参数写入审计表。
- 操作日志写入失败不影响原业务请求；它用于后台追踪和排障，不替代商品库存事务。
